...
public class NodeService {
	...
	private Response response(String[] strings) {
		return Response.ok(stringListRep(strings)).header(Time.TIME_STAMP, Time.advanceTime()).build();
	}
	...
	public static JAXBElement<String[]> stringListRep(String[] strings) {
		return new JAXBElement<String[]>(nsTableRow, String[].class, strings);
	}
	...
	// GET: /dht?key=KEY Retrieve the bindings for a key at a node
	...
	// PUT: /Add a binding for a key at a node
	public Response setKeyValue(String key, String value) {
		try {
			advanceTime();
			info("setKeyValue");
			dht.add(key, value);
	...
	// GET: /dht/finger?id=ID Get the closest preceding finger table entry of ID
	public Response getFinger(String id) {
		advanceTime();
		info("getFinger");
		return response(dht.closestPrecedingFinger(Integer.parseInt(id)));
	}
